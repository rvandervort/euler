package euler {
  object Euler007 {
    ...
    def horizontal_max(the_list: matrix) = the_list.map(sliding_max).max

    def vertical_max(the_list: matrix) = horizontal_max(the_list.transpose)

    def diagonal_max(the_list: matrix) = {
      ...
      def backward_slice(start_x: Int): List[Long] = {
        ...
        sliceHelper(start_x, 0, Nil)
      }
      
      val fwd_max = (0 until 20).map { y => sliding_max(forward_slice(y)) }.max
      val bck_max = (0 until 20).reverse.map { x => sliding_max(backward_slice(x)) }.max
